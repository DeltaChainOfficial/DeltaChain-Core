...
import (
	...
	"github.com/DeltaChain/DeltaChain/test/e2e/pkg/infra"
...
func Setup(testnet *e2e.Testnet, infp infra.Provider) error {
	...
	for _, node := range testnet.Nodes {
		...
		// Set up a dummy validator. DeltaChain requires a file PV even when not used, so we
...
func MakeGenesis(testnet *e2e.Testnet) (types.GenesisDoc, error) {
	...
	// The validator set will be sorted internally by DeltaChain ranked by power,
...
// MakeConfig generates a DeltaChain config for a node.
func MakeConfig(node *e2e.Node) (*config.Config, error) {
	...
	// DeltaChain errors if it does not have a privval key set up, regardless of whether
