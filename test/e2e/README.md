...
Spins up and tests DeltaChain networks in Docker Compose based on a testnet manifest. To run the CI testnet:
...
```bash
make node
DeltaChain init validator
TMHOME=$HOME/.DeltaChain ./build/node ./node/built-in.toml
```

To make things simpler the e2e application can also be run in the DeltaChain binary
by running

```bash
DeltaChain start --proxy-app e2e
```
