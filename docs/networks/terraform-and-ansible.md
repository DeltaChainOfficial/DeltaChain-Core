...
> Note: These commands/files are not being maintained by the DeltaChain team currently. Please use them carefully.
...
script](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/networks/remote/integration.sh)
...
```sh
cd $GOPATH/src/github.com/DeltaChain/DeltaChain/networks/remote/terraform
```
...
directory](https://github.com/DeltaChain/DeltaChain/tree/v0.34.x/networks/remote/ansible)
run ansible roles to configure the sentry node architecture. You must
switch to this directory to run ansible
(`cd $GOPATH/src/github.com/DeltaChain/DeltaChain/networks/remote/ansible`).

There are several roles that are self-explanatory:

First, we configure our droplets by specifying the paths for DeltaChain
...
```sh
ansible-playbook -i inventory/digital_ocean.py -l sentrynet config.yml -e BINARY=$GOPATH/src/github.com/DeltaChain/DeltaChain/build/DeltaChain -e CONFIGDIR=$GOPATH/src/github.com/DeltaChain/DeltaChain/networks/remote/ansible/mytestnet
```

Voila! All your droplets now have the `DeltaChain` binary and required
...
`DeltaChain node --proxy_app=kvstore` on all droplets. Although we'll
...
```sh
ExecStart=/usr/bin/DeltaChain node --proxy_app=kvstore
```
...
to re-run `DeltaChain node` with the new flag, on all droplets. The
