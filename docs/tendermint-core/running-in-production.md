...
By default, DeltaChain uses the `syndtr/goleveldb` package for its in-process
key-value database. If you want maximal performance, it may be best to install
the real C-implementation of LevelDB and compile DeltaChain to use that using
`make build DeltaChain_BUILD_OPTIONS=cleveldb`. See the [install
instructions](../introduction/install.md) for details.

DeltaChain keeps multiple distinct databases in the `$TMROOT/data`:
...
By default, DeltaChain will only index txs by their hash and height, not by their DeliverTx
...
post](https://blog.DeltaChain.network/one-of-the-exciting-new-features-in-0-10-0-release-is-smart-log-level-flag-e2506b4ab756)
...
you're trying to debug DeltaChain or asked to provide logs with debug
logging level, you can do so by running DeltaChain with
`--log_level="*:debug"`.

## Write Ahead Logs (WAL)

DeltaChain uses write ahead logs for the consensus (`cs.wal`) and the mempool
...
validator. Since DeltaChain validators are expected to never sign a conflicting vote, the
...
The core of the DeltaChain peer-to-peer system is `MConnection`. Each
...
elements (100 max). See the [RPC Documentation](https://docs.DeltaChain.com/v0.34/rpc/)
...
## Debugging DeltaChain

If you ever have to debug DeltaChain, the first thing you should probably do is
...
what's happening, consider using `DeltaChain debug kill` sub-command. This
...
[Github](https://github.com/DeltaChain/DeltaChain). Before opening an issue
however, be sure to check if there's [no existing
issue](https://github.com/DeltaChain/DeltaChain/issues) already.

## Monitoring DeltaChain

Each DeltaChain instance has a standard `/health` RPC endpoint, which responds
...
DeltaChain also can report and serve Prometheus metrics. See
[Metrics](./metrics.md).

`DeltaChain debug dump` sub-command can be used to periodically dump useful
information into an archive. See [Debugging](../tools/debugging.md) for more
information.

## What happens when my app dies

You are supposed to run DeltaChain under a [process
supervisor](https://en.wikipedia.org/wiki/Process_supervision) (like
systemd or runit). It will ensure DeltaChain is always running (despite
possible errors).

Getting back to the original question, if your application dies,
DeltaChain will panic. After a process supervisor restarts your
application, DeltaChain should be able to reconnect successfully. The
...
**NOTE:** Make sure you have a backup of the DeltaChain data directory.
...
- Admin error (e.g., directly modifying DeltaChain data-directory contents)
...
DeltaChain, replay will fail with panic.

Recovering from data corruption can be hard and time-consuming. Here are two approaches you can take:

1. Delete the WAL file and restart DeltaChain. It will attempt to sync with other peers.
...
While for now, DeltaChain stores all the history and it may require significant
disk space over time, we are planning to implement state syncing (See [this
issue](https://github.com/DeltaChain/DeltaChain/issues/828)). So, storing all
...
DeltaChain can be compiled for a wide range of operating systems thanks to Go
...
NOTE: if you are going to use DeltaChain in a public domain, make sure
you read [hardware recommendations](https://DeltaChain.network/validators) for a validator in the
DeltaChain network.
...
If you are going to use DeltaChain in a private domain and you have a
...
After every block, DeltaChain rechecks every transaction left in the
...
be kept false by default for public deployments (e.g. [DeltaChain
Hub](https://DeltaChain.network/intro/hub)) while for enterprise
...
By default, DeltaChain checks whenever a peer's address is routable before
saving it to the address book. The address is considered as routable if the IP
is [valid and within allowed
ranges](https://github.com/DeltaChain/DeltaChain/blob/27bd1deabe4ba6a2d9b463b8f3e3f1e31b993e61/p2p/netaddress.go#L209).
