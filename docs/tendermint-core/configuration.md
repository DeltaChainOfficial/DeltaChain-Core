...
DeltaChain Core can be configured via a TOML file in
...
The default configuration file create by `DeltaChain init` has all
the parameters set with their default values. It will look something
like the file below, however, double check by inspecting the
`config.toml` created with your version of `DeltaChain` installed:
...
# "$HOME/.DeltaChain" by default, but could be changed via $TMHOME env variable
# or --home cmd flag.
...
# Migth be either absolute path or path related to DeltaChain's config directory.
...
#
# For DeltaChain SDK-based chains, trust_period should usually be about 2/3 of the unbonding time (~2
...
Note after the block H, DeltaChain creates something we call a "proof block"
...
default (`0`), DeltaChain will be creating empty blocks even in the absence of
...
DeltaChain will only create blocks if there are transactions, or after waiting
