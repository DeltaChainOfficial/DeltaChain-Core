...
applications. DeltaChain provides unique speed and security properties for
light client applications.

See our [light
package](https://pkg.go.dev/github.com/DeltaChain/DeltaChain/light?tab=doc).
...
[merkle proofs](https://github.com/DeltaChain/spec/blob/953523c3cb99fdb8c8f7a2d21e3a99094279e9de/spec/blockchain/encoding.md#iavl-tree).

## Properties

- You get the full collateralized security benefits of DeltaChain; no
  need to wait for confirmations.
- You get the full speed benefits of DeltaChain; transactions
  commit instantly.
...
[Trust Options](https://pkg.go.dev/github.com/DeltaChain/DeltaChain/light?tab=doc#TrustOptions)
...
DeltaChain comes with a built-in `DeltaChain light` command, which can be used
to run a light client proxy server, verifying DeltaChain RPC. All calls that
...
interface as a full DeltaChain node.

You can start the light client proxy server by running `DeltaChain light <chainID>`,
...
```bash
$ DeltaChain light supernova -p tcp://233.123.0.140:26657 \
...
For additional options, run `DeltaChain light --help`.
