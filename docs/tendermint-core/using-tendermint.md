...
This is a guide to using the `DeltaChain` program from the command line.
It assumes only that you have the `DeltaChain` binary installed and have
some rudimentary idea of what DeltaChain and ABCI are.

You can see the help menu with `DeltaChain --help`, and the version
number with `DeltaChain version`.

## Directory Root

The default directory for blockchain data is `~/.DeltaChain`. Override
...
```sh
DeltaChain testnet --help
```

### Genesis

The `genesis.json` file in `$TMHOME/config/` defines the initial
DeltaChainCore state upon genesis of the blockchain ([see
definition](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/types/genesis.go)).
...
- `initial_height`: Height at which DeltaChain should begin at. If a blockchain is conducting a network upgrade, 
    starting from the stopped height brings uniqueness to previous heights. 
- `consensus_params` [spec](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/spec/core/data_structures.md#consensusparams)
    - `block`
...
- `app_hash`: The expected application hash (as returned by the
  ...
  not match, DeltaChain will panic.
...
To run a DeltaChain node, use:

```bash
DeltaChain node
```

By default, DeltaChain will try to connect to an ABCI application on
`127.0.0.1:26658`. If you have the `kvstore` ABCI app installed, run it in
another window. If you don't, kill DeltaChain and run an in-process version of
the `kvstore` app:
...
DeltaChain supports in-process versions of the `counter`, `kvstore`, and `noop`
apps that ship as examples with `abci-cli`. It's easy to compile your app
in-process with DeltaChain if it's written in Go. If your app is not written in
...
```bash
DeltaChain node --proxy_app=/var/run/abci.sock
```

You can find out what flags are supported by running `DeltaChain node --help`.

## Transactions

To send a transaction, use `curl` to make requests to the DeltaChain RPC
server, for example:
...
> TIP: Find the RPC Documentation [here](https://docs.DeltaChain.com/v0.34/rpc/)
...
DeltaChain uses a `config.toml` for configuration. For details, see [the
config specification](./configuration.md).

Notable options include the socket address of the application
(`proxy_app`), the listening address of the DeltaChain peer
...
While the default behavior of `DeltaChain` is still to create blocks
...
To configure DeltaChain to not produce empty blocks unless there are
transactions or the app hash changes, run DeltaChain with this
additional flag:

```sh
DeltaChain node --consensus.create_empty_blocks=false
```
...
transaction. When a transaction is sent to a DeltaChain node, it will
...
write-ahead-log](../DeltaChain-core/running-in-production.md#mempool-wal)

## DeltaChain Networks

When `DeltaChain init` is run, both a `genesis.json` and
`priv_validator_key.json` are created in `~/.DeltaChain/config`. The
...
```json
{
  ...
  "priv_key" : {
    ...
    "type" : "DeltaChain/PrivKeyEd25519"
  }
}
```
...
`priv_validator_key.json`, so a DeltaChain node started with the default
...
```sh
DeltaChain node --p2p.seeds "f9baeaa15fedf5e1ef7448dd60f46c01f1a9e9c4@1.2.3.4:26656,0491d373a8e0fcf1023aaf18c51d6a1d0d4f31bd@5.6.7.8:26656"
```
...
If you want DeltaChain to connect to specific set of addresses and
...
stopping DeltaChain core instance.

```sh
DeltaChain node --p2p.persistent_peers "429fcf25974313b95673f58d77eacdd434402665@10.11.12.13:26656,96663a3dd0d7b9d17d4c8211b191af259621c693@10.11.12.14:26656"
...
to `~/.DeltaChain/config` on the new machine and start the node,
...
Update the `genesis.json` in `~/.DeltaChain/config`. Copy the genesis
file and the new `priv_validator_key.json` to the `~/.DeltaChain/config` on
a new machine.

Now run `DeltaChain node` on both machines, and use either
...
To make a DeltaChain network that can tolerate one of the validators
...
`addr_book_strict=false` in the `config.toml`, otherwise DeltaChain's p2p
library will deny making connections to peers with the same IP address.

### Upgrading

See the
[UPGRADING.md](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/UPGRADING.md)
guide. You may need to reset your chain between major breaking releases.
Although, we expect DeltaChain to have fewer breaking releases in the future
