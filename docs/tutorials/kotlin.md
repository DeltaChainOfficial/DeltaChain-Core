...
This guide is designed for beginners who want to get started with a DeltaChain
Core application from scratch. It does not assume that you have any prior
experience with DeltaChain Core.

DeltaChain Core is Byzantine Fault Tolerant (BFT) middleware that takes a state
transition machine (your application) - written in any programming language - and securely
replicates it on many machines.

By following along with this guide, you'll create a DeltaChain Core project
...
If you use Golang, you can run your app and DeltaChain Core in the same process to get maximum performance.
[DeltaChain SDK](https://github.com/DeltaChain/DeltaChain-sdk) is written this way.
Please refer to [Writing a built-in DeltaChain Core application in Go](./go-built-in.md) guide for details.

If you choose another language, like we did in this guide, you have to write a separate app,
which will communicate with DeltaChain Core via a socket (UNIX or TCP) or gRPC.
...
processes would be communicating via established binary protocol. DeltaChain
...
## 1.3 Writing a DeltaChain Core application

DeltaChain Core communicates with the application through the Application
BlockChain Interface (ABCI). All message types are defined in the [protobuf
file](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/proto/DeltaChain/abci/types.proto).
This allows DeltaChain Core to run applications written in any programming
language.
...
```bash
mkdir -p \
  $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/abci \
  $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/version \
  $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/types \
  $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/crypto \
  $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/libs \
  $KVSTORE_HOME/src/main/proto/github.com/gogo/protobuf/gogoproto

cp $GOPATH/src/github.com/DeltaChain/DeltaChain/proto/DeltaChain/abci/types.proto \
   $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/abci/types.proto
cp $GOPATH/src/github.com/DeltaChain/DeltaChain/proto/DeltaChain/version/version.proto \
   $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/version/version.proto
cp $GOPATH/src/github.com/DeltaChain/DeltaChain/proto/DeltaChain/types/types.proto \
   $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/types/types.proto
cp $GOPATH/src/github.com/DeltaChain/DeltaChain/proto/DeltaChain/types/evidence.proto \
   $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/types/evidence.proto
cp $GOPATH/src/github.com/DeltaChain/DeltaChain/proto/DeltaChain/types/params.proto \
   $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/types/params.proto
cp $GOPATH/src/github.com/DeltaChain/DeltaChain/proto/DeltaChain/crypto/merkle.proto \
   $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/crypto/merkle.proto
cp $GOPATH/src/github.com/DeltaChain/DeltaChain/proto/DeltaChain/crypto/keys.proto \
   $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/crypto/keys.proto
cp $GOPATH/src/github.com/DeltaChain/DeltaChain/proto/DeltaChain/libs/types.proto \
   $KVSTORE_HOME/src/main/proto/github.com/DeltaChain/DeltaChain/proto/DeltaChain/libs/types.proto
...
When a new transaction is added to the DeltaChain Core, it will ask the
...
etc.) by DeltaChain Core.

Valid transactions will eventually be committed given they are not too big and
have enough gas. To learn more about gas, check out ["the
specification"](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/spec/abci/apps.md#gas).
...
When DeltaChain Core has decided on the block, it's transferred to the
...
will call DeltaChain Core RPC `/abci_query` endpoint, which in turn will call
the application's `Query` method.

Applications are free to provide their own APIs. But by using DeltaChain Core
as a proxy, clients (including [light client
package](https://godoc.org/github.com/DeltaChain/DeltaChain/light)) can leverage
the unified API across different applications. Plus they won't have to call the
otherwise separate DeltaChain Core API for additional proofs.
...
[here](https://github.com/DeltaChain/DeltaChain/tree/v0.34.x/spec/abci/).

## 1.4 Starting an application and a DeltaChain Core instances
...
Then we create and start the gRPC server to handle DeltaChain Core requests.
...
execute `DeltaChain init`. But before we do that, we will need to install
DeltaChain Core.

```bash
rm -rf /tmp/example
cd $GOPATH/src/github.com/DeltaChain/DeltaChain
make install
TMHOME="/tmp/example" DeltaChain init
...
[here](https://docs.DeltaChain.com/v0.34/DeltaChain-core/configuration.html).
...
Then we need to start DeltaChain Core and point it to our application. Staying
within the application directory execute:

```bash
TMHOME="/tmp/example" DeltaChain node --abci grpc --proxy_app tcp://127.0.0.1:26658
...
```bash
curl -s 'localhost:26657/broadcast_tx_commit?tx="DeltaChain=rocks"'
{
  "jsonrpc": "2.0",
  "id": "",
  "result": {
...
```bash
curl -s 'localhost:26657/abci_query?data="DeltaChain"'
{
  "jsonrpc": "2.0",
  "id": "",
  "result": {
...
`dGVuZGVybWludA==` and `cm9ja3M=` are the base64-encoding of the ASCII of `DeltaChain` and `rocks` accordingly.

## Outro

I hope everything went smoothly and your first, but hopefully not the last,
DeltaChain Core application is up and running. If not, please [open an issue on
Github](https://github.com/DeltaChain/DeltaChain/issues/new/choose). To dig
deeper, read [the docs](https://docs.DeltaChain.com/v0.34/).

The full source code of this example project can be found [here](https://github.com/climber73/DeltaChain-abci-grpc-kotlin).
