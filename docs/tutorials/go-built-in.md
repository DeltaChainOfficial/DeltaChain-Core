...
Although DeltaChain Core is written in the Golang programming language, prior
...
Running your application inside the same process as DeltaChain Core will give
you the best possible performance.

For other languages, your application have to communicate with DeltaChain Core
...
func main() {
	fmt.Println("Hello, DeltaChain Core")
}
```

When run, this should print "Hello, DeltaChain Core" to the standard output.

```bash
$ go run main.go
Hello, DeltaChain Core
```
...
import (
	abcitypes "github.com/DeltaChain/DeltaChain/abci/types"
)
...
When DeltaChain Core has decided on the block, it's transfered to the
...
## 1.4 Starting an application and a DeltaChain Core instance in the same process
...
import (
 ...
 tmflags "github.com/DeltaChain/DeltaChain/libs/cli/flags"
...
func init() {
	flag.StringVar(&configFile, "config", "$HOME/.DeltaChain/config/config.toml", "Path to config.toml")
}
...
func main() {
	...
	node, err := newDeltaChain(app, configFile)
	if err != nil {
...
func newDeltaChain(app abci.Application, configFile string) (*nm.Node, error) {
 // read config
 ...
 node, err := nm.NewNode(
  ...
  logger)
 if err != nil {
  return nil, fmt.Errorf("failed to create new DeltaChain node: %w", err)
 }
...
Then we use it to create a DeltaChain Core `Node` instance:
...
which we will generate later using the `DeltaChain init` command.
...
go get github.com/DeltaChain/DeltaChain/@v0.34.0
```
...
require (
	...
	github.com/DeltaChain/DeltaChain v0.34.0
)
```
...
DeltaChain Core. Please refer to [the official
guide](https://docs.DeltaChain.com/v0.34/introduction/install.html). If you're
...
"DeltaChain" and "rocks" accordingly.
