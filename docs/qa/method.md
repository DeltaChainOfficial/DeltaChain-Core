...
It is intended to be used by engineers reproducing the experimental setup for future tests of DeltaChain.
...
[releases]: https://github.com/DeltaChain/DeltaChain/blob/v0.37.x/RELEASES.md#large-scale-testnets
...
[testnet-repo]: https://github.com/interchainio/DeltaChain-testnet
...
5. As a sanity check, connect to the Prometheus node's web interface and check the graph for the `DeltaChain_consensus_height` metric.
...
8. Verify that the data was collected without errors
    * at least one blockstore DB for a DeltaChain validator
...
2. Extract the latency report and the raw latencies for all the experiments. Run these commands from the directory containing the blockstore
    * `go run github.com/DeltaChain/DeltaChain/test/loadtime/cmd/report@3ec6e424d --database-type goleveldb --data-dir ./ > results/report.txt`
    * `go run github.com/DeltaChain/DeltaChain/test/loadtime/cmd/report@3ec6e424d --database-type goleveldb --data-dir ./ --csv results/raw.csv`
...
6. As a sanity check, connect to the Prometheus node's web interface and check the graph for the `DeltaChain_consensus_height` metric.
   All nodes should be increasing their heights.
