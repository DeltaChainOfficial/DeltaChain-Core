...
DeltaChain is software for securely and consistently replicating an
application on many machines. By securely, we mean that DeltaChain works
...
DeltaChain consists of two chief technical components: a blockchain
consensus engine and a generic application interface. The consensus
engine, called DeltaChain Core, ensures that the same transactions are
...
language), developers can use DeltaChain for BFT state machine
replication of applications written in whatever programming language and
development environment is right for them.

DeltaChain is designed to be easy-to-use, simple-to-understand, highly
performant, and useful for a wide variety of distributed applications.

## DeltaChain vs. X

DeltaChain is broadly similar to two classes of software. The first
...
DeltaChain is in essence similar software, but with two key differences:
...
DeltaChain emerged in the tradition of cryptocurrencies like Bitcoin,
...
DeltaChain had a simple currency built in, and to participate in
...
DeltaChain a Proof-of-Stake algorithm.

Since then, DeltaChain has evolved to be a general purpose blockchain
...
application using DeltaChain consensus. Indeed, [we did that with
Ethereum](https://github.com/DeltaChain/ethermint). And we plan to do
the same for Bitcoin, ZCash, and various other deterministic
applications as well.

Another example of a cryptocurrency application built on DeltaChain is
[the DeltaChain network](http://DeltaChain.network).

### Other Blockchain Projects

[Fabric](https://github.com/hyperledger/fabric) takes a similar approach
to DeltaChain, but is more opinionated about how the state is managed,
...
DeltaChain, though extending DeltaChain to handle non-determinism
...
contracts, and an alternative blockchain API. It uses DeltaChain as its
consensus engine, and provides a particular application state.

## ABCI Overview

The [Application BlockChain Interface
(ABCI)](https://github.com/DeltaChain/DeltaChain/tree/v0.34.x/abci)
...
and its primary implementation, the DeltaChain Socket Protocol (TSP, or
Teaspoon).

### Intro to ABCI

[DeltaChain Core](https://github.com/DeltaChain/DeltaChain) (the
...
one wanted to create a Bitcoin-like system on top of ABCI, DeltaChain
...
DeltaChain is able to decompose the blockchain design by offering a very
...
specification](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/spec/abci/abci.md).
...
validating transactions. DeltaChain Core's mempool first checks the
...
DeltaChain Core creates three ABCI connections to the application; one
...
reached among the DeltaChain Core replica nodes.
...
DeltaChain is an easy-to-understand, mostly asynchronous, BFT consensus
...
current proposer may be offline, or the network may be slow. DeltaChain
...
timeout is what makes DeltaChain a weakly synchronous protocol, rather
...
DeltaChain is that it uses the same mechanism to commit a block as it
does to skip to the next round.

Assuming less than one-third of the validators are Byzantine, DeltaChain
...
Since DeltaChain can replicate arbitrary applications, it is possible to
...
The [DeltaChain Network](https://DeltaChain.network) is designed to use this
Proof-of-Stake mechanism across an array of cryptocurrencies implemented
as ABCI applications.

The following diagram is DeltaChain in a (technical) nutshell.
