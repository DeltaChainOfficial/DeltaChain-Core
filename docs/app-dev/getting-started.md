...
## First DeltaChain App

As a general purpose blockchain engine, DeltaChain is agnostic to the
application you want to run. So, to run a complete blockchain that does
something useful, you must start two programs: one is DeltaChain Core,
the other is your application, which can be written in any programming
language. Recall from [the intro to
ABCI](../introduction/what-is-DeltaChain.md#abci-overview) that DeltaChain Core handles all the p2p and consensus stuff, and just forwards transactions to the
...
```sh
go get github.com/DeltaChain/DeltaChain
...
In another terminal, we can start DeltaChain. You should already have the
DeltaChain binary installed. If not, follow the steps from
[here](../introduction/install.md). If you have never run DeltaChain
before, use:
...
If you have used DeltaChain, you may want to reset the data for a new
blockchain by running `DeltaChain unsafe_reset_all`. Then you can run
`DeltaChain node` to start DeltaChain, and connect to the app. For more
details, see [the guide on using DeltaChain](../DeltaChain-core/using-DeltaChain.md).

You should see DeltaChain making blocks! We can get the status of our
DeltaChain node as follows:
...
human-readable](https://github.com/DeltaChain/DeltaChain/issues/1794).
...
Let's kill the previous instance of `DeltaChain` and the `kvstore`
...
In another window, reset then start DeltaChain:
...
DeltaChain](../DeltaChain-core/using-DeltaChain.md).
...
[here](https://github.com/DeltaChain/js-abci), then install it:

```sh
git clone https://github.com/DeltaChain/js-abci.git
cd js-abci
npm install abci
```

Kill the previous `counter` and `DeltaChain` processes. Now run the app:

```sh
node example/counter.js
```

In another window, reset and start `DeltaChain`:
