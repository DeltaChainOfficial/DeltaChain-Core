...
## DeltaChain debug kill

DeltaChain comes with a `debug` sub-command that allows you to kill a live
DeltaChain process while collecting useful information in a compressed archive.
...
```bash
DeltaChain debug kill <pid> </path/to/out.zip> --home=</path/to/app.d>
```
...
```bash
DeltaChain debug dump </path/to/out> --home=</path/to/app.d>
```
