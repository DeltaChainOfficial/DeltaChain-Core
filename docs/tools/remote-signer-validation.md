...
Located under the `tools/tm-signer-harness` folder in the [DeltaChain
repository](https://github.com/DeltaChain/DeltaChain).

The DeltaChain remote signer test harness facilitates integration testing
between DeltaChain and remote signers such as
[KMS](https://github.com/DeltaChain/kms). Such remote signers allow for signing
of important DeltaChain messages using
...
[DeltaChain](https://github.com/DeltaChain/DeltaChain).

## Building

From the `tools/tm-signer-harness` directory in your DeltaChain source
...
`tools/tm-signer-harness` directory of your DeltaChain source repo, simply run:
...
you how to execute its tests against [KMS](https://github.com/DeltaChain/kms).
...
See the [KMS repo](https://github.com/DeltaChain/kms) for details on how to set
...
example, we will simply export a signing key from our local DeltaChain instance.

```bash
# Will generate all necessary DeltaChain configuration files, including:
# - ~/.DeltaChain/config/priv_validator_key.json
# - ~/.DeltaChain/data/priv_validator_state.json
DeltaChain init

# Extract the signing key from our local DeltaChain instance
tm-signer-harness extract_key \      # Use the "extract_key" command
    -tmhome ~/.DeltaChain \          # Where to find the DeltaChain home directory
...
chain_id = "test-chain-0XwP5E"         # The DeltaChain chain ID for which KMS will be signing (found in ~/.DeltaChain/config/genesis.json).
reconnect = true                       # true is the default
secret_key = "./secret_connection.key" # Where to find our secret connection key.

[[providers.softsign]]
id = "test-chain-0XwP5E"               # The DeltaChain chain ID for which KMS will be signing (same as validator.chain_id above).
...
tm-signer-harness run \             # The "run" command executes the tests
    ...
    -tmhome ~/.DeltaChain           # Where to find our DeltaChain configuration/data files.
```

If the current version of DeltaChain and KMS are compatible, `tm-signer-harness`
