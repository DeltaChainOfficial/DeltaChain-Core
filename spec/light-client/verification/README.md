...
generated by the DeltaChain consensus), the following inequality holds: `Header.Time < now + CLOCK_DRIFT`,
...
For the purpose of this light client specification, we assume that the DeltaChain Full Node
exposes the following functions over DeltaChain RPC:
...
- *Light Client Completeness*: If a header `h` was correctly generated by an instance of DeltaChain consensus (and its age is less than the trusted period),
then the light client should eventually set `trust(h)` to `true`.

- *Light Client Accuracy*: If a header `h` was *not generated* by an instance of DeltaChain consensus, then the light client should never set `trust(h)` to true.
...
(that is were not generated by an instance of DeltaChain consensus), it may submit (a subset of) the headers it has seen as evidence of misbehavior.
...
*Assumption*: Initially, the light client has a header `inithead` that it trusts, that is, `inithead` was correctly generated by the DeltaChain consensus.
...
*Remark.* To prove the invariant, we will have to prove that the light client only trusts headers that were correctly generated by DeltaChain consensus.
...
- as `v` is correct up to now, it followed the DeltaChain consensus protocol at least up to signing `untrustedHeader` => `untrustedHeader` was correctly generated.
...
*Verification Condition:* We may need a DeltaChain invariant stating that if `untrustedSignedHeader.Header.Height = trustedHeader.Height + 1` then
