...
This document explains how DeltaChain Peers are identified and how they connect to one another.

For details on peer discovery, see the [peer exchange (PEX) reactor doc](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/spec/reactors/pex/pex.md).

## Peer Identity

DeltaChain peers are expected to maintain long-term persistent identities in the form of a public key.
...
two handshakes are performed: one for authenticated encryption, and one for DeltaChain versioning.
Both handshakes have configurable timeouts (they should complete quickly).

### Authenticated Encryption Handshake

DeltaChain implements the Station-to-Station protocol
...
- create a new Merlin Transcript with the string "DeltaChain_SECRET_CONNECTION_TRANSCRIPT_HASH"
...
- generate two keys to use for encryption (sending and receiving) and a challenge for authentication as follows:
    ...
    `DeltaChain_SECRET_CONNECTION_KEY_AND_CHALLENGE_GEN`
...
### DeltaChain Version Handshake

The DeltaChain Version Handshake allows the peers to exchange their NodeInfo:
