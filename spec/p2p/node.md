...
A DeltaChain P2P network has different kinds of nodes with different requirements for connectivity to one another.
This document describes what kind of nodes DeltaChain should enable and how they should work.
...
See [the peer-exchange docs](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/spec/reactors/pex/pex.md) for
...
- a list of seeds, which can be provided to DeltaChain via config file or flags,
...
The values `H` and `HASH` must be received and corroborated by means external to DeltaChain, and specific to the user - ie. via the user's trusted social consensus.
...
dials those peers, and runs the DeltaChain protocols with those it successfully connects to.

When the peer catches up to height H, it ensures the block hash matches HASH.
If not, DeltaChain will exit, and the user must try again - either they are connected
