...
ABCI is the interface between DeltaChain (a state-machine replication engine)
and your application (the actual state machine). It consists of a set of
_methods_, each with a corresponding `Request` and `Response`message type. 
To perform state-machine replication, DeltaChain calls the ABCI methods on the 
ABCI application by sending the `Request*` messages and receiving the `Response*` messages in return.

All ABCI messages and methods are defined in [protocol buffers](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/proto/abci/types.proto). 
This allows DeltaChain to run with applications written in many programming languages.
