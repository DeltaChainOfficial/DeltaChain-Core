...
ABCI applications can run either within the _same_ process as the DeltaChain
state-machine replication engine, or as a _separate_ process from the state-machine
replication engine. When run within the same process, DeltaChain will call the ABCI
application methods directly as Go method calls.

When DeltaChain and the ABCI application are run as separate processes, DeltaChain
...
indicates to DeltaChain that an error occurred.

These methods also return a `Codespace` string to DeltaChain. This field is
...
issue that DeltaChain has no reasonable way to handle. If there is an error in one
of these methods, the application must crash to ensure that the error is safely
handled by an operator.

The handling of non-zero response codes by DeltaChain is described below

### CheckTx

The `CheckTx` ABCI method controls what transactions are considered for inclusion in a block.
When DeltaChain receives a `ResponseCheckTx` with a non-zero `Code`, the associated
transaction will be not be added to DeltaChain's mempool or it will be removed if
it is already included.

### DeliverTx

The `DeliverTx` ABCI method delivers transactions from DeltaChain to the application.
When DeltaChain recieves a `ResponseDeliverTx` with a non-zero `Code`, the response code is logged.
...
When DeltaChain receives a `ResponseQuery` with a non-zero `Code`, this code is 
...
Events returned via these ABCI methods do not impact DeltaChain consensus in any way
and instead exist to power subscriptions and queries of DeltaChain state.
...
index flag notifies the DeltaChain indexer to index the attribute. The value of
...
DeltaChain's security model relies on the use of "evidence". Evidence is proof of
malicious behaviour by a network participant. It is the responsibility of DeltaChain
to detect such malicious behaviour. When malicious behavior is detected, DeltaChain
...
information can be found in either [data structures](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/spec/core/data_structures.md)
or [accountability](https://github.com/DeltaChain/DeltaChain/blob/v0.34.x/spec/light-client/accountability/)

## Determinism

ABCI applications must implement deterministic finite-state machines to be
securely replicated by the DeltaChain consensus engine. This means block execution
...
like DeltaChain Core. The application must only change its state based on input
...
See [#56](https://github.com/DeltaChain/abci/issues/56) for original discussion.
...
The first time a new blockchain is started, DeltaChain calls
...
A DeltaChain node that receives a request for snapshots from a peer will call
...
begins restoring it, DeltaChain will fetch snapshot "chunks" from existing nodes.
...
* **Request**:
    ...
    |---------------|--------|------------------------------------------|--------------|
    | version       | string | The DeltaChain software semantic version | 1            |
    | block_version | uint64 | The DeltaChain Block Protocol version    | 2            |
    | p2p_version   | uint64 | The DeltaChain P2P Protocol version      | 3            |
    | abci_version  | string | The DeltaChain ABCI semantic version     | 4            |
...
* **Usage**:
    ...
    * Used to sync DeltaChain with the application during a handshake
    that happens on startup.
    * The returned `app_version` will be included in the Header of every block.
    * DeltaChain expects `last_block_app_hash` and `last_block_height` to
...
* **Usage**:
    ...
    * This allows the app to decide if it wants to accept the initial validator
    set proposed by DeltaChain (ie. in the genesis file), or if it wants to use
...
* **Usage**:
    ...
    * The header contains the height, timestamp, and more - it exactly matches the
    DeltaChain block header. We may seek to generalize this in the future.
...
* **Usage**:
    ...
    * DeltaChain attributes no other value to the response code
...
* **Usage**:
    ...
    * When `DeliverTx` is called, the application must execute the transaction in full before returning control to DeltaChain.
...
* **Usage**:
    * `OfferSnapshot` is called when bootstrapping a node using state sync. The application may
    accept or reject snapshots as appropriate. Upon accepting, DeltaChain will retrieve and
...
* **Request**:
    ...
    | Name   | Type   | Description                                                                 | Field Number |
    |--------|--------|-----------------------------------------------------------------------------|--------------|
    | index  | uint32 | The chunk index, starting from `0`. DeltaChain applies chunks sequentially. | 1            |
...
* **Usage**:
    * The application can choose to refetch chunks and/or ban P2P peers as appropriate. DeltaChain
    ...
    * When all chunks have been accepted, DeltaChain will make an ABCI `Info` call to verify that
    ...
    network.
    * If DeltaChain is unable to retrieve the next chunk after some time (e.g. because no suitable
...
* **Usage**:
    ...
    * Used to tell DeltaChain to update the validator set
...
* **Fields**:
    ...
    | format   | uint32 | An application-specific snapshot format, allowing applications to version their snapshot data format and make backwards-incompatible changes. DeltaChain does not interpret this. | 2            |
    | chunks   | uint32 | The number of chunks in the snapshot. Must be at least 1 (even if empty).                                                                                                         | 3            |
    | hash     | bytes  | TAn arbitrary snapshot hash. Must be equal only for identical snapshots across nodes. DeltaChain does not interpret the hash, it only compares them.                              | 3            |
