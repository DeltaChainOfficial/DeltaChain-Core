...
- [Crash Recovery](#crash-recovery) - handshake protocol to synchronize
  DeltaChain and the application on startup.
- [State Sync](#state-sync) - rapid bootstrapping of new nodes by restoring state machine snapshots

## Connection State

Since DeltaChain maintains four concurrent ABCI connections, it is typical
...
[default in-process ABCI client](https://github.com/DeltaChain/DeltaChain/blob/v0.34.4/abci/client/local_client.go#L18)
and the
[default Go ABCI
server](https://github.com/DeltaChain/DeltaChain/blob/v0.34.4/abci/server/socket_server.go#L32)
use global locks across all connections, so they are not
concurrent at all. This means if your app is written in Go, and compiled in-process with DeltaChain
...
every block. It also allows DeltaChain to send the current block hash
...
has run a successful Commit) so that it can tell DeltaChain where to
...
Before `Commit` is called, DeltaChain locks and flushes the mempool so that no new messages will
...
Before calling `Commit`, DeltaChain will lock and flush the mempool connection,
...
Finally, after re-checking transactions in the mempool, DeltaChain will unlock
...
and for initialization when DeltaChain first starts up (both described further
below).
...
DeltaChain Core currently uses the Query connection to filter peers upon
connecting, according to IP address or node ID. For instance,
returning non-OK ABCI response to either of the following queries will
cause DeltaChain to not connect to the corresponding peer:
...
the difference credited back. DeltaChain adopts a similar abstraction,
though uses it only optionally and weakly, allowing applications to define
their own sense of the cost of execution.

In DeltaChain, the [ConsensusParams.Block.MaxGas](../proto/types/params.proto) limits the amount of `gas` that can be used in a block.
...
When `MaxGas > -1`, DeltaChain enforces the following rules:
...
Note that DeltaChain does not currently enforce anything about Gas in the consensus, only the mempool.
This means it does not guarantee that committed blocks satisfy these rules!
It is the application's responsibility to return non-zero response codes when gas limits are exceeded.

The `GasUsed` field is ignored completely by DeltaChain. That said, applications should enforce:
...
proposal. See [#1861](https://github.com/DeltaChain/DeltaChain/issues/1861).
...
semantically meaningless to DeltaChain.

`Events` include any events for the execution, though since the transaction has not
been committed yet, they are effectively ignored by DeltaChain.

### DeliverTx

DeliverTx is the workhorse of the blockchain. DeltaChain sends the
...
`Events` include any events for the execution, which DeltaChain will use to index
...
If the list is empty, DeltaChain will use the validators loaded in the genesis
file.
If the list returned by `InitChain` is not empty, DeltaChain will use its contents as the validator set.
This way the application can set the initial validator set for the
blockchain.

### EndBlock

Updates to the DeltaChain validator set can be made by returning
`ValidatorUpdate` objects in the `ResponseEndBlock`:

```protobuf
message ValidatorUpdate {
  DeltaChain.crypto.keys.PublicKey pub_key
  int64 power
}
...
This is enforced by DeltaChain consensus.
...
This is *not* enforced by DeltaChain consensus.
It is left to the app to enforce (ie. if txs are included past the
limit, they should return non-zero codes). It is used by DeltaChain to limit the
...
If ConsensusParams is nil, DeltaChain will use the params loaded in the genesis
file. If ConsensusParams is not nil, DeltaChain will use it.
...
If ConsensusParams nil, DeltaChain will do nothing.
If ConsensusParam is not nil, DeltaChain will use it.
...
of queries on application state. DeltaChain makes use of Query to filter new peers
...
Note DeltaChain has technically no requirements from the Query
...
The DeltaChain block header includes a number of hashes, each providing an
...
Some applications (eg. Ethereum, DeltaChain-SDK) have multiple "levels" of Merkle trees,
...
When DeltaChain connects to a peer, it sends two queries to the ABCI application
...
If either of these queries return a non-zero ABCI code, DeltaChain will refuse
...
DeltaChain only uses `/p2p`, for filtering peers. For more advanced use, see the
implementation of
[Query in the DeltaChain-SDK](https://github.com/DeltaChain/DeltaChain-sdk/blob/v0.23.1/baseapp/baseapp.go#L333).

## Crash Recovery

On startup, DeltaChain calls the `Info` method on the Info Connection to get the latest
...
We now distinguish three heights, and describe how DeltaChain syncs itself with
the app.

```md
storeBlockHeight = height of the last block DeltaChain saw a commit for
stateBlockHeight = height of the last block for which DeltaChain completed all
...
Note also DeltaChain never calls Commit on an ABCI app twice for the same height.
...
This happens if we crashed after the app finished Commit but before DeltaChain saved the state.
...
When a new node is running state sync and discovering snapshots, DeltaChain will query an existing
...
light client verification of the restored snapshot: a set of DeltaChain RPC servers, and a
...
is accepted DeltaChain will fetch and apply the snapshot chunks. After all chunks have been
successfully applied, DeltaChain verifies the app's `AppHash` against the chain using the light
...
it, and so on. DeltaChain will keep discovering and offering snapshots until one is accepted or
the application aborts.

#### Snapshot Restoration

Once a snapshot has been accepted via `OfferSnapshot`, DeltaChain begins downloading chunks from
...
If DeltaChain fails to fetch a chunk after some time, it will reject the snapshot and try a
...
Once all chunks have been accepted, DeltaChain issues an `Info` ABCI call to retrieve the
`LastBlockAppHash`. This is compared with the trusted app hash from the chain, retrieved and
verified using the light client. DeltaChain also checks that `LastBlockHeight` corresponds to the
...
counteract this by asking DeltaChain to ban peers. As a last resort, node operators can use
P2P configuration options to whitelist a set of trusted peers that can provide valid snapshots.

#### Transition to Consensus

Once the snapshots have all been restored, DeltaChain gathers additional information necessary for
...
Once the state machine has been restored and DeltaChain has gathered this additional
