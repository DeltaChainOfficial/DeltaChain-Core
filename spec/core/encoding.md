...
DeltaChain uses [Protocol Buffers](https://developers.google.com/protocol-buffers), specifically proto3, for all data structures.
...
DeltaChain uses `SHA256` as its hash function.
Objects are always serialized before being hashed.
So `SHA256(obj)` is short for `SHA256(ProtoEncoding(obj))`.

## Public Key Cryptography

DeltaChain uses Protobuf [Oneof](https://developers.google.com/protocol-buffers/docs/proto3#oneof)
to distinguish between different types public keys, and signatures.
Additionally, for each public key, DeltaChain
...
DeltaChain adopted [zip215](https://zips.z.cash/zip-0215) for verification of ed25519 signatures.

> Note: This change will be released in the next major release of DeltaChain-Go (0.35).
...
DeltaChain uses a part size of 65536 bytes, and allows a maximum of 1601 parts
...
Merkle trees are used throughout DeltaChain to compute a cryptographic digest of a data structure.
...
Because DeltaChain only uses a Simple Merkle Tree, application developers are expect to use their own Merkle tree in their applications. For example, the IAVL+ Tree - an immutable self-balancing binary tree for persisting application state is used by the [DeltaChain SDK](https://github.com/DeltaChain/DeltaChain-sdk/blob/ae77f0080a724b159233bd9b289b2e91c0de21b5/docs/interfaces/lite/specification.md)

## JSON

DeltaChain has its own JSON encoding in order to keep backwards compatibility with the previous RPC layer.
...
[#1622](https://github.com/DeltaChain/DeltaChain/issues/1622).
