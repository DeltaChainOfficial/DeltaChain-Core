...
Here we describe the data structures in the DeltaChain blockchain and the rules for validating them.

The DeltaChain blockchains consists of a short list of data types:
...
| Data       | [Data](#data)                  | Data contains a list of transactions. The contents of the transaction is unknown to DeltaChain.                                                                                      | This field can be empty or populated, but no validation is performed. Applications can perform validation on individual transactions prior to block creation using [checkTx](../abci/abci.md#checktx).
...
Part defines a part of a block. In DeltaChain blocks are broken into `parts` for gossip.
...
DeltaChain uses the [Google.Protobuf.Timestamp](https://developers.google.com/protocol-buffers/docs/reference/google.protobuf#google.protobuf.Timestamp)
...
| Name | Type                       | Description            | Validation                                                                  |
|------|----------------------------|------------------------|-----------------------------------------------------------------------------|
| Txs  | Matrix of bytes ([][]byte) | Slice of transactions. | Validation does not occur on this field, this data is unknown to DeltaChain |
...
(see [ADR-25](https://github.com/DeltaChain/DeltaChain/blob/main/docs/architecture/adr-025-commit.md)).
...
Signatures in DeltaChain are raw bytes representing the underlying signature.
...
Evidence in DeltaChain is used to indicate breaches in the consensus by a validator.

More information on how evidence works in DeltaChain can be found [here](../consensus/evidence.md)
