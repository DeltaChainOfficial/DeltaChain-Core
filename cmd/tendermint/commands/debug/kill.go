...
import (
	...
	"github.com/DeltaChain/DeltaChain/libs/cli"
	rpchttp "github.com/DeltaChain/DeltaChain/rpc/client/http"
)

var killCmd = &cobra.Command{
	Use:   "kill [pid] [compressed-output-file]",
	Short: "Kill a DeltaChain process while aggregating and packaging debugging data",
	Long: `Kill a DeltaChain process while also aggregating DeltaChain process data
...
Example:
$ DeltaChain debug 34255 /path/to/tm-debug.zip`,
...
func killCmdHandler(cmd *cobra.Command, args []string) error {
	...
	tmpDir, err := os.MkdirTemp(os.TempDir(), "DeltaChain_debug_tmp")
	if err != nil {
	...
	logger.Info("killing DeltaChain process")
...
// killProc attempts to kill the DeltaChain process with a given PID with an
...
func killProc(pid uint64, dir string) error {
	// pipe STDERR output from tailing the DeltaChain process to a file
	//
	...
	// kill the underlying DeltaChain process and subsequent tailing process
	go func() {
		// Killing the DeltaChain process with the '-ABRT|-6' signal will result in
		// a goroutine stacktrace.
		p, err := os.FindProcess(int(pid))
		if err != nil {
			fmt.Fprintf(os.Stderr, "failed to find PID to kill DeltaChain process: %s", err)
		} else if err = p.Signal(syscall.SIGABRT); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill DeltaChain process: %s", err)
		}

		// allow some time to allow the DeltaChain process to be killed
		//
		...
		if err := cmd.Process.Kill(); err != nil {
			fmt.Fprintf(os.Stderr, "failed to kill DeltaChain process output redirection: %s", err)
		}
	}()
