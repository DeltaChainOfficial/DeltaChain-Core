...
import (
	...
	tmmath "github.com/DeltaChain/DeltaChain/libs/math"
	tmos "github.com/DeltaChain/DeltaChain/libs/os"
	"github.com/DeltaChain/DeltaChain/light"
	lproxy "github.com/DeltaChain/DeltaChain/light/proxy"
	lrpc "github.com/DeltaChain/DeltaChain/light/rpc"
	dbs "github.com/DeltaChain/DeltaChain/light/store/db"
...
var LightCmd = &cobra.Command{
	Use:   "light [chainID]",
	Short: "Run a light client proxy server, verifying DeltaChain rpc",
	Long: `Run a light client proxy server, verifying DeltaChain rpc.

All calls that can be tracked back to a block header by a proof
will be verified before passing them back to the caller. Other than
that, it will present the same interface as a full DeltaChain node.
...
for applications built w/ DeltaChain SDK).
`,
	RunE: runProxy,
	Args: cobra.ExactArgs(1),
	Example: `light DeltaChainhub-3 -p http://52.57.29.196:26657 -w http://public-seed-node.DeltaChainhub.certus.one:26657
...
func init() {
	...
	LightCmd.Flags().StringVarP(&primaryAddr, "primary", "p", "",
		"connect to a DeltaChain node at this address")
	LightCmd.Flags().StringVarP(&witnessAddrsJoined, "witnesses", "w", "",
		"DeltaChain nodes to cross-check the primary node, comma-separated")
	LightCmd.Flags().StringVar(&home, "home-dir", os.ExpandEnv(filepath.Join("$HOME", ".DeltaChain-light")),
	...
	if primaryAddr == "" { // check to see if we can start from an existing state
		...
		if primaryAddr == "" {
			return errors.New("no primary address was provided nor found. Please provide a primary (using -p)." +
				" Run the command: DeltaChain light --help for more information")
		}
	} else {
