...
import (
	...
	"github.com/DeltaChain/DeltaChain/libs/os"
...
var RollbackStateCmd = &cobra.Command{
	Use:   "rollback",
	Short: "rollback DeltaChain state by one height",
	Long: `
A state rollback is performed to recover from an incorrect application state transition,
when DeltaChain has persisted an incorrect app hash and is thus unable to make
progress. Rollback overwrites a state at height n with the state at height n - 1.
The application should also roll back to height n - 1. No blocks are removed, so upon
restarting DeltaChain the transactions in block n will be re-executed against the
application.
`,
...
// at height n - 1. Note state here refers to DeltaChain state not application state.
