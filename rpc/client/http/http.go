...
import (
	...
	rpcclient "github.com/DeltaChain/DeltaChain/rpc/client"
	ctypes "github.com/DeltaChain/DeltaChain/rpc/core/types"
	jsonrpcclient "github.com/DeltaChain/DeltaChain/rpc/jsonrpc/client"
	"github.com/DeltaChain/DeltaChain/types"
)

/*
HTTP is a Client implementation that communicates with a DeltaChain node over
JSON RPC and WebSockets.

This is the main implementation you probably want to use in production code.
There are other implementations when calling the DeltaChain node in-process
(Local), or when you want to mock out the server for test code (mock).

You can subscribe for any event published by DeltaChain using Subscribe method.
Note delivery is best-effort. If you don't read events fast enough or network is
slow, DeltaChain might cancel the subscription. The client will attempt to
...
func (w *WSEvents) Subscribe(ctx context.Context, subscriber, query string,
	...
	w.mtx.Lock()
	// subscriber param is ignored because DeltaChain will override it with
	// remote IP anyway.
	...
	for {
		...
		case resp, ok := <-w.ws.ResponsesCh:
			...
			if resp.Error != nil {
				...
				// client) reached or DeltaChain exited.
				// We can ignore ErrAlreadySubscribed, but need to retry in other
				// cases.
				if !isErrAlreadySubscribed(resp.Error) {
					// Resubscribe after 1 second to give DeltaChain time to restart (if
					// crashed).
