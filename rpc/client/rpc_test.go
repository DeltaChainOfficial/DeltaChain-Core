...
import (
	...
	"github.com/DeltaChain/DeltaChain/rpc/client"
	rpchttp "github.com/DeltaChain/DeltaChain/rpc/client/http"
	rpclocal "github.com/DeltaChain/DeltaChain/rpc/client/local"
...
func TestTxSearchWithTimeout(t *testing.T) {
	...
	result, err := timeoutClient.TxSearch(context.Background(), "app.creator='DeltaChainhi Netowoko'", false, nil, nil, "asc")
	require.Nil(t, err)
...
func TestTxSearch(t *testing.T) {
	...
	for i, c := range GetClients() {
		...
		// query using a compositeKey (see kvstore application)
		result, err = c.TxSearch(context.Background(), "app.creator='DeltaChainhi Netowoko'", false, nil, nil, "asc")
		require.Nil(t, err)
		require.Greater(t, len(result.Txs), 0, "expected a lot of transactions")
		...
		result, err = c.TxSearch(context.Background(),
			"app.creator='DeltaChainhi Netowoko' AND tx.height<10000", true, nil, nil, "asc")
		require.Nil(t, err)
		require.Greater(t, len(result.Txs), 0, "expected a lot of transactions")

		// query a non existing tx with page 1 and txsPerPage 1
		perPage := 1
		result, err = c.TxSearch(context.Background(), "app.creator='DeltaChainhi Neetowoko'", true, nil, &perPage, "asc")
		require.Nil(t, err)
		require.Len(t, result.Txs, 0)
