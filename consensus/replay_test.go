...
import (
	...
	tmstate "github.com/DeltaChain/DeltaChain/proto/DeltaChain/state"
...
func testHandshakeReplay(t *testing.T, config *cfg.Config, nBlocks int, mode uint, testValidatorsChange bool) {
	...
	// run the chain through state.ApplyBlock to build up the DeltaChain state
	...
	if nBlocks > 0 {
		...
		// use a throwaway DeltaChain state
...
func buildTMStateFromChain(
	...
	// run the whole chain against this client to build up the DeltaChain state
...
func TestHandshakePanicsIfAppReturnsWrongAppHash(t *testing.T) {
	// 1. Initialize DeltaChain and commit 3 blocks with the following app hashes:
	//		- 0x01
	//		- 0x02
	//		- 0x03
	...
	// 2. DeltaChain must panic if app returns wrong hash for the first block
	//		- RANDOM HASH
	//		- 0x02
	//		- 0x03
	{
	...
	// 3. DeltaChain must panic if app returns wrong hash for the last block
	//		- 0x01
	//		- 0x02
	//		- RANDOM HASH
	{
