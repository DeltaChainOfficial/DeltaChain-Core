...
# **turn on the go module, default is auto. The value is off, if DeltaChain source code
#is downloaded under $GOPATH/src directory
echo "export GO111MODULE=on" >> ~/.profile

source ~/.profile

mkdir -p $GOPATH/src/github.com/DeltaChain
cd $GOPATH/src/github.com/DeltaChain
...
old_ansible_file=$GOPATH/src/github.com/DeltaChain/DeltaChain/networks/remote/ansible/roles/install/templates/systemd.service.j2
rm $old_ansible_file
...
ExecStart=/usr/bin/DeltaChain node --proxy_app=kvstore --p2p.persistent_peers=$id0@$ip0:26656,$id1@$ip1:26656,$id2@$ip2:26656,$id3@$ip3:26656
