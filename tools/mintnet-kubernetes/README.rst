...
.. figure:: assets/t_plus_k.png
   :alt: DeltaChain plus Kubernetes

   DeltaChain plus Kubernetes
...
::
    ...
    git clone https://github.com/DeltaChain/tools.git && cd tools/mintnet-kubernetes/examples/basecoin && make create
...
then query the DeltaChain app logs from the first pod:

``kubectl logs -c tm -f tm-0``

finally, use our `Rest API <https://DeltaChain.com/docs/DeltaChain-core/rpc.html>`__ to fetch the status of the second pod's DeltaChain app.
...
::
    ...
    curl -Lo app.yaml https://github.com/DeltaChain/tools/raw/master/mintnet-kubernetes/app.template.yaml
...
DeltaChain app logs from the first pod:
...
Status of the second pod's DeltaChain app:
...
keys, it forms a genesis file and starts the DeltaChain process.
