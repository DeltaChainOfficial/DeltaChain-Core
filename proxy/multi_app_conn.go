...
import (
	...
	tmlog "github.com/DeltaChain/DeltaChain/libs/log"
...
// AppConns is the DeltaChain's interface to the application that consists of
...
func (app *multiAppConn) OnStart() error {
	...
	// Kill DeltaChain if the ABCI application crashes.
...
func (app *multiAppConn) killTMOnClientError() {
	killFn := func(conn string, err error, logger tmlog.Logger) {
		logger.Error(
			fmt.Sprintf("%s connection terminated. Did the application crash? Please restart DeltaChain", conn),
			"err", err)
		killErr := tmos.Kill()
		if killErr != nil {
